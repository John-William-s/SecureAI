...
class FileEncryptorUI(tk.Toplevel):
    ...
    def __init__(self, master):
        ...
        encrypt_button = tk.Button(button_frame, text="Encrypt", font=self.button_font, bg=SUCCESS_COLOR, fg=BUTTON_FG, command=self.encrypt_file_action)
        encrypt_button.pack(side="left", expand=True, ipady=10, padx=(0, 10))
        
        decrypt_button = tk.Button(button_frame, text="Decrypt", font=self.button_font, bg=BUTTON_BG, fg=BUTTON_FG, command=self.decrypt_file_action)
    ...
    def encrypt_file_action(self):
        ...
        try:
            ...
            # Save the encrypted file (salt + encrypted data)
            new_filepath = self.file_path + ".enc"
            with open(new_filepath, 'wb') as f:
                f.write(salt)
                f.write(encrypted_data)
            
            self.status_label.config(text=f"Success! File encrypted to {os.path.basename(new_filepath)}", fg=SUCCESS_COLOR)
            messagebox.showinfo("Success", f"File encrypted successfully!\nSaved as: {new_filepath}")
    ...
    def decrypt_file_action(self):
        ...
        if not self.file_path.endswith(".enc"):
            messagebox.showerror("Error", "Please select a file with a '.enc' extension to decrypt.")
            return
        ...
        try:
            ...
            original_filepath = self.file_path.rsplit('.enc', 1)[0]
            with open(original_filepath, 'wb') as f:
                f.write(decrypted_data)
            
            self.status_label.config(text=f"Success! File decrypted to {os.path.basename(original_filepath)}", fg=SUCCESS_COLOR)
            messagebox.showinfo("Success", f"File decrypted successfully!\nSaved as: {original_filepath}")
